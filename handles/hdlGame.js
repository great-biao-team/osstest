...
exports.getAll = function(req, res, next){
	Game.getAll(function(err, obj){
		...
		//find 返回数组 若未找到则返回数组 []
			...
			return res.status(404).json(func.formJsonMsg(0,"Empty set : Game"));
		else
			return res.status(200).json(func.formJsonMsg(1, obj));
	});
};

exports.getByGameId = function(req, res, next){
	Game.getOneByGameId(req.params.game_id, function(err, obj){
		if(err)
			return next(err);
		else if(!obj)
			return res.status(404).json(func.formJsonMsg(0,"Game not found : " + req.params.game_id));
		else 
...
exports.add = function(req, res, next){
	if(!req.body.game_id || !req.body.game_name)
...
exports.update = function(req, res, next){
	if(req.body.game_id)
		return res.status(403).json(func.formJsonMsg(0,"Invalid param : game_id"));
	else
		Game.updateByGameId(
			req.params.game_id,
			req.body,
