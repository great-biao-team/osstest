...
exports.getAll = function(req, res, next){
	RolePrivilege.getAll(function(err, obj){
		if(err) 
			return next(err);
		//find 返回数组 若未找到则返回数组 []
		else if(obj.length == 0)
			return res.status(404).json(func.formJsonMsg(0,"Empty set : RolePrivilege"));
		else
...
exports.getByRoleId = function(req, res, next){
	RolePrivilege.getSomeByRoleId(req.params.role_id, function(err, obj){
		if(err)
			return next(err);
		else if(obj.length == 0)
			return res.status(404).json(func.formJsonMsg(0,"RolePrivilege not found : " + req.params.role_id));
		else 
			return res.status(200).json(func.formJsonMsg(1, obj));
	});
};
